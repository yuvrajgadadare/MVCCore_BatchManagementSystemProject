...
namespace MVCCore_BatchManagementSystemProject.Areas.Student.Controllers
{
    public class CourseDetailsController : Controller
    {
        ...
        public CourseDetailsController(IStudentService studentService)
        {
        ...
        public IActionResult Index()
        {
            if (HttpContext.Session.GetInt32("StudentId") != null)
            {
               ...
               //  TblstudentDetail st = studentService.GetStudentDetails(studentId);
                List<StudentRegistrationModel> lst = studentService.GetStudentRegistrations().Where(e => e.StudentId.Equals(studentId)).ToList();
                TblstudentDetail s = studentService.GetStudentDetails(studentId);
                return View(s);
            }
            else
            {
